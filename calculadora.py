...
def conv_graus_radiano(dado_1):
    if '°' in dado_1:
        dado_1 = dado_1.replace('°','')
        radianos = math.radians(float(dado_1))
        return radianos
    else:
        return dado_1
...
def cientifica(entrada_str):
    ...
    if dado[0] == 'tan':
        ...
        resultado = round(math.cos(float(valor_radiano)),5)
    elif dado[0] == 'sin':
        resultado = round(math.sin(float(valor_radiano)),5)
    elif dado[0] == 'sqrt':
        numero = float(dado[1])
        if numero > 0:
            resultado = math.sqrt(float(numero))
        else:
            resultado = cmath.sqrt(float(numero))
    elif dado[0] == 'log':
...
def igual():
    entrada_str = entrada.get()
    entrada.delete(0,'end')
    expressao = entrada_str.replace('×','*')
    ...
    try:
        if entrada_str.find('(') > 0:
            resultado = cientifica(entrada_str)
...
btn_pow = Button(janela,text='pow',command=lambda:pressionar('^'),fg=cor4,bg=cor3,activeforeground=cor4,activebackground=cor3,font=fontfamily,borderwidth=1,relief=SOLID)
btn_pow.grid(row=3,column=1,sticky=E+W,ipady=5)

btn_rem = Button(janela,text='mod',command=lambda:pressionar('mod'),fg=cor4,bg=cor3,activeforeground=cor4,activebackground=cor3,font=fontfamily,borderwidth=1,relief=SOLID)
...
btn_igual = Button(janela,text='=',command=igual,fg=cor1,bg=cor4,activeforeground=cor1,activebackground=cor4,font=fontfamily,borderwidth=1,relief=SOLID)
btn_igual.grid(row=9,column=0,columnspan=3,sticky=E+W,ipady=5)

btn_simb_graus = Button(janela,text='°',command=lambda:pressionar('°'),fg=cor4,bg=cor3,activeforeground=cor4,activebackground=cor3,font=fontfamily,borderwidth=1,relief=SOLID)
btn_simb_graus.grid(row=9,column=3,sticky=E+W,ipady=5)
